...
<Aside type="caution">
  `bids.send` actually emails the customer. If you don't want to give your AI assistant the power to
...
<Aside type="caution">
  `pay_apps.approve` is a workflow action. The approval timestamp and the user are recorded in the
...
<Aside type="caution">
  Webhook signing secrets are returned exactly once — on `webhooks.create` and
  `webhooks.rotate_secret`. Every other tool redacts them. There is no API to fetch an existing
