...
<Aside type="caution">
Hourly rate and total cost are computed by the server from your labor-rate table. The MCP tool ignores any rate or cost fields you pass — payroll math isn't something an AI assistant gets to set.
</Aside>
...
<Aside type="caution">
Webhook signing secrets are returned exactly once — on `webhooks.create` and `webhooks.rotate_secret`. Every other tool redacts them. There is no API to fetch an existing secret; if you lose it, rotate.
</Aside>
