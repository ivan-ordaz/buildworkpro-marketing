...
The consent screen lets you control exactly what Claude can do. Read scopes (`*:read`) are safe — they only let Claude fetch data. Write scopes (`*:write`) let Claude create or update records. Workflow scopes (`bids:send`, `pay_apps:approve`, `change_orders:approve`) let Claude take actions that affect your business. Grant only what you trust the model with. You can revoke access at any time in **Settings -> Developer -> Connected apps**.
</Aside>
