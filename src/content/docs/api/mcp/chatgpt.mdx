...
The exact UI for adding MCP servers is evolving as OpenAI rolls out their MCP support broadly. Menu labels and the Apps directory layout may change. If the steps above don't match what you see, check OpenAI's connector documentation at [platform.openai.com/docs](https://platform.openai.com/docs) for the current path. The server URL and OAuth flow are stable — only the ChatGPT-side install UX is moving.
</Aside>
...
- ChatGPT's training data does not include your BuildWorkPro records. Tool calls fetch live data each time. If a record changed in the last few seconds, ask ChatGPT to refetch.
- LLMs occasionally call the wrong tool or pass wrong arguments. Always review write actions (creating a contact, sending a bid) before confirming.
- Don't paste sensitive customer info into the chat itself — let the tools fetch it. Anything you type into the chat is sent to OpenAI's servers.
</Aside>
