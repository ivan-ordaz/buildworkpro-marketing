...
This page documents the most common webhook events. The full set of subscribable event types is enumerated in the `eventTypes` enum of the create-endpoint request (`POST /api/v1/webhook-endpoints`) in the API reference.
...
<Aside type="note">
  Treat `data` as a snapshot, not the source of truth. If you need the latest state of the record,
  fetch it from the REST API using `data.id`.
</Aside>

<Aside type="tip">
  The sections below detail the most common events. Event types not documented individually here --
  including the `*.updated` and `*.deleted` variants and the full `lead.*` set -- use this same
  envelope: `data` carries the resource snapshot and `previous_attributes` accompanies field changes.
  You can subscribe to any type in the `eventTypes` enum of `POST /api/v1/webhook-endpoints`.
</Aside>
