...
BuildWorkPro emits 29 event types at launch, spanning bids, projects, pay applications, change orders, contacts, and leads. The cards below highlight the most common ones; the full payload shape for each is documented in the [Event Catalog](/api/webhooks/events/).
...
```json
{
  "data": {
    ...
    "eventTypes": ["bid.accepted"],
    "isActive": true,
    "signingSecret": "bwh_secret_5f3c1b2a..."
  }
}
```
...
<Aside type="note">
  Always verify signatures against the **raw request body bytes**, never the parsed JSON. Even a
  whitespace difference will break the HMAC. See [Verifying Signatures](/api/webhooks/signatures/)
  for full details.
</Aside>
