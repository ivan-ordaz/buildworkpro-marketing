...
<Aside type="tip">
  Tunnels rotate URLs every time you restart them (unless you have a paid plan). Either pin a stable
  subdomain on your tunnel provider, or `PATCH /api/v1/webhook-endpoints/{id}` with the new URL each
  session.
</Aside>
...
- **Response body** -- captured up to 1 KB (redacted), on failed deliveries only.
...
<Aside type="note">
  Replay is allowed even after a delivery has reached `exhausted`. The only case it fails is `409
  conflict` -- the linked endpoint has been soft-deleted. Recreate the endpoint and the delivery
  will need to be re-emitted from the source action.
</Aside>
