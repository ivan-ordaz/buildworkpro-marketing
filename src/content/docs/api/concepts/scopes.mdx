...
| `pay_apps:mark_paid`    | Confirm payment of an approved pay application -- manager only. |
...
| `audit:read`      | Read API audit log entries and consumption metrics for the calling tenant.       |
| `settings:read`   | Read organization-wide configuration such as project templates.                  |
| `settings:write`  | Manage organization-wide configuration such as project templates. Admin-only.    |
...
<Aside type="note">
  `offline_access` is OAuth-specific -- it's the signal that a refresh token should be issued
  alongside the access token. API keys don't expire and don't need it.
</Aside>
