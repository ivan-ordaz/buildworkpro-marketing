...
<Aside type="caution">
  Generate a new UUID per logical operation. If your code uses the same key for two different things
  (e.g., creating a contact and creating a lead), the second request will fail with `409
  idempotency_key_reused` because the bodies don't match.
</Aside>
