...
Errors from the v1 API follow [RFC 9457 (Problem Details for HTTP APIs)](https://datatracker.ietf.org/doc/html/rfc9457). They're returned with `Content-Type: application/problem+json` and a machine-readable `type` URL whose trailing fragment is a stable error code you can switch on in your client.
...
<Aside type="note">
  Always log `request_id` server-side for every failed call. When you open a support ticket, include
  the request id and timestamp -- it lets us pull the exact request from logs without guesswork.
</Aside>
...
- Switch on the trailing fragment of `type` (e.g. `#validation_failed`), not `status` or `title`. The codes are stable; titles may be reworded.
- Treat `internal_error`, `service_unavailable`, and `maintenance` as retriable (see the Retry column above). Retry once for `internal_error`; use exponential backoff for `service_unavailable` and `maintenance`.
