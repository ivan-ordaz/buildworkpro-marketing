...
See also: [Projects overview](/docs/projects/overview/) — the in-app view of the projects this prompt summarizes.
...
<Aside type="caution" title="Watch out for">
  - The summaries are LLM-generated. ChatGPT may invent or misstate small details (a phase number, a
  date) — verify before sharing externally with a customer or partner. - Long project names, many
  concurrent phases, or unusually-rich notes may exceed ChatGPT's display budget and get truncated.
  Ask ChatGPT to expand a specific project if the line is too short. - The prompt fetches live data
  each time you invoke it. If you ran the prompt yesterday and want to compare, ask ChatGPT to keep
  the previous output in context: "Compare today's summary to yesterday's — what changed?" - Share
  the brief, not the chat URL. The chat history may include other tool call output you don't want to
  expose.
</Aside>
