...
See also: [Leads](/docs/crm/leads/) and [Contacts](/docs/crm/contacts/) — where synced leads land in the app and the contact records they link to.
...
<Aside type="caution" title="Watch out for">
  - **The lead schema is name-and-stage-only at minimum.** Email isn't a first-class field on
  `leads` — link the lead to a contact (which does carry email) via `contactId` if you need email
  storage. Until then, the search-by-email dedupe relies on the email being mentioned in another
  searchable field, or on you also pre-creating a contact. - **Scope: `leads:write`.** A key with
  only `leads:read` will get `403 forbidden`. The dedupe lookup additionally needs `leads:read`. -
  **Validation failures come back as 422 with a per-field error list.** Surface the messages in your
  form UX so the user knows what to fix; never echo the raw error envelope to the browser.
</Aside>
