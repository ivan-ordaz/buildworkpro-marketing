...
See also: [Sending a bid](/docs/bids/sending/) — how a bid reaches `sent` status and what acceptance looks like in the app.
...
2. **Stand up a receiver that verifies the signature.**
   ...
   const seen = new Set(); // swap for a database or other durable store in production
...
<Aside type="caution" title="Watch out for">
  - **You MUST verify against the raw request body.** If `express.json()` runs before your route,
  `req.body` is already a parsed object and the signature will never match. Mount `express.raw()` on
  the webhook path only. - **Dedupe by `event.id`, not delivery id.** Same event re-delivered
  (manual replay or an automatic retry after a 5xx) carries the same `id`. Store seen ids for at
  least 7 days — the retry schedule extends out to 24 hours. - **Acknowledge fast.** Return `200`
  within a few seconds; offload Slack/CRM work to a background queue. Slow receivers cause retries
  and backpressure on the dispatcher.
</Aside>
